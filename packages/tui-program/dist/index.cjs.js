...
var colors = require('@geia/tui-colors');
var tuiEvents = require('@geia/tui-events');
var tuiTerminfoParser = require('@geia/tui-terminfo-parser');
...
class GpmClient extends tuiEvents.EventEmitter {
  constructor(options) {
    ...
    try {
      path = fs__default['default'].readlinkSync('/proc/' + pid + '/fd/0');
    } catch (e) {}
    ...
    if (tty) {
      fs__default['default'].stat(GPM_SOCKET, function (err, stat) {
...
const functionKeyCodeReAnywhere = new RegExp('(?:\x1b+)(O|N|\\[|\\[\\[)(?:' + ['(\\d+)(?:;(\\d+))?([~^$])', '(?:M([@ #!a`])(.)(.))', // mouse
'(?:1;)?(\\d+)?([a-zA-Z])'].join('|') + ')');
const functionKeyCodeRe = new RegExp('^' + functionKeyCodeReAnywhere.source);
const escapeCodeReAnywhere = new RegExp([functionKeyCodeReAnywhere.source, metaKeyCodeReAnywhere.source, /\x1b./.source].join('|'));
...
class Program extends tuiEvents.EventEmitter {
  constructor(options = {}) {
    ...
    this.sc = this.saveCursor;
    this.rc = this.restoreCursor;
    this.enter_alt_charset_mode = this.smacs;
    this.as = this.smacs;
    this.exit_alt_charset_mode = this.rmacs;
    this.ae = this.rmacs;
    this.cuu = this.cursorUp;
    this.up = this.cursorUp;
    this.cud = this.cursorDown;
    this.down = this.cursorDown;
    this.cuf = this.cursorForward;
    this.right = this.cursorForward;
    this.forward = this.cursorForward;
    this.cub = this.cursorBackward;
    this.left = this.cursorBackward;
    this.back = this.cursorBackward;
    this.cup = this.cursorPos;
    this.pos = this.cursorPos;
    this.ed = this.eraseInDisplay;
    this.el = this.eraseInLine;
    this.sgr = this.charAttributes;
    this.attr = this.charAttributes;
    this.fg = this.setForeground;
    this.bg = this.setBackground;
    this.dsr = this.deviceStatus;
    this.ich = this.insertChars;
    this.cnl = this.cursorNextLine;
    this.cpl = this.cursorPrecedingLine;
    this.cha = this.cursorCharAbsolute;
    this.il = this.insertLines;
    this.dl = this.deleteLines;
    this.dch = this.deleteChars;
    this.ech = this.deleteChars;
    this.hpa = this.charPosAbsolute;
    this.hpr = this.HPositionRelative;
    this.da = this.sendDeviceAttributes;
    this.vpa = this.linePosAbsolute;
    this.vpr = this.VPositionRelative;
    this.hvp = this.HVPosition;
    this.sm = this.setMode;
    ...
    this.decstbm = this.setScrollRegion;
    this.csr = this.setScrollRegion;
    this.scA = this.saveCursorA;
    this.rcA = this.restoreCursorA;
    this.cht = this.cursorForwardTab;
    this.su = this.scrollUp;
    this.sd = this.scrollDown;
    this.cbt = this.cursorBackwardTab;
    this.rep = this.repeatPrecedingCharacter;
    this.tbc = this.tabClear;
    this.mc = this.mediaCopy;
    this.print_screen = this.mc0;
    this.ps = this.mc0;
    this.prtr_on = this.mc5;
    this.po = this.mc5;
    this.prtr_off = this.mc4;
    this.pf = this.mc4;
    this.prtr_non = this.mc5p;
    this.pO = this.mc5p;
    this.decstr = this.softReset;
    this.rs2 = this.softReset;
    this.decrqm = this.requestAnsiMode;
    this.decrqmp = this.requestPrivateMode;
    this.decscl = this.setConformanceLevel;
    this.decll = this.loadLEDs;
    this.decscusr = this.setCursorStyle;
    this.decsca = this.setCharProtectionAttr;
    this.deccara = this.setAttrInRectangle;
    this.decrara = this.reverseAttrInRectangle;
    this.decswbv = this.setWarningBellVolume;
    this.decsmbv = this.setMarginBellVolume;
    this.deccra = this.copyRectangle;
    this.decefr = this.enableFilterRectangle;
    this.decreqtparm = this.requestParameters;
    this.decsace = this.selectChangeExtent;
    this.decfra = this.fillRectangle;
    this.decelr = this.enableLocatorReporting;
    this.decera = this.eraseRectangle;
    this.decsle = this.setLocatorEvents;
    this.decsera = this.selectiveEraseRectangle;
    ...
    if (options.log) {
      this._logger = fs__default['default'].createWriteStream(options.log);
  ...
  log() {
    return this._log('LOG', util__default['default'].format.apply(util__default['default'], arguments));
  }
  ...
  setupTput() {
    ...
    const tput = this.tput = new tuiTerminfoParser.Tput({
  ...
  _listenOutput() {
    ...
    this.output.on('resize', this.output._resizeHandler = function () {
      Program.instances.forEach(function (program) {
        ...
        const time = typeof program.options.resizeTimeout === 'number' ? program.options.resizeTimeout : 300;
  ...
  _attr(param, val) {
    ...
    switch (param) {
      ...
      default:
        // 256-color fg and bg
        if (param[0] === '#') {
          param = param.replace(/#(?:[0-9a-f]{3}){1,2}/i, colors__namespace.match);
        }
        ...
        if (m) {
          ...
          color = colors__namespace.reduce(color, this.tput.colors);
