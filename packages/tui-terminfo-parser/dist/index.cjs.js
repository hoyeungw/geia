...
var alias = require('@geia/tui-alias');
...
var alias__namespace = /*#__PURE__*/_interopNamespace(alias);
...
class Tput {
  ...
  constructor(options) {
    ...
    this._readTermcap = Tput.prototype.readTermcap; // Convert ACS unicode characters to the
  ...
  parseTerminfo(data, file) {
    ...
    info.names = parts;
    info.desc = desc;
    info.dir = path__default['default'].resolve(file, '..', '..');
    info.file = file;
    i += h.namesSize - 1; // Names is nul-terminated.

    assert__default['default'].equal(data[i], 0);
    i++; // Booleans Section
    ...
    Object.keys(info.strings).forEach(function (key) {
      ...
      assert__default['default'](j < data.length);
  ...
  parseExtended(data) {
    ...
    _strings.forEach(function (offset, k) {
      ...
      assert__default['default'](j < data.length); // Find out where the string table ends by
  ...
  _compile(info, key, str) {
    ...
    if (key === 'init_file' || key === 'reset_file') {
      try {
        str = fs__default['default'].readFileSync(str, 'utf8');
      ...
      for (j = 0; j < fields.length; j++) {
        ...
        if (j === 0) {
          ...
          term = {
            ...
            names: names,
            desc: names.pop(),
            file: ~file.indexOf(path__default['default'].sep) ? path__default['default'].resolve(file) : file,
            termcap: true
          };
  ...
  getAll() {
    const dir = this._prefix(),
          list = asort(fs__default['default'].readdirSync(dir)),
          infos = [];

    list.forEach(function (letter) {
      const terms = asort(fs__default['default'].readdirSync(path__default['default'].resolve(dir, letter)));
  ...
  GetConsoleCP() {
    ...
    try {
      ...
      ccp = cp__default['default'].execFileSync(process.env.WINDIR + '\\system32\\chcp.com', [], {
  ...
  readTermcap(term) {
    ...
    if (~term.indexOf(path__default['default'].sep) && (terms = this._tryCap(path__default['default'].resolve(term)))) {
      term_ = path__default['default'].basename(term).split('.')[0];
  ...
  readTerminfo(term) {
    ...
    file = path__default['default'].normalize(this._prefix(term));
    data = fs__default['default'].readFileSync(file);
...
Tput._prefix = Tput.cpaths;
Tput._tprefix = Tput.cpaths;
Tput._alias = alias__namespace;
Tput.alias = {};
Tput.aliasMap = {};
...
function merge(a, b) {
  return Object.keys(b).forEach(key => a[key] = b[key]), a;
}
...
function tryRead(file) {
  ...
  if (!file) return '';
  file = path__default['default'].resolve.apply(path__default['default'], arguments);

  try {
    return fs__default['default'].readFileSync(file, 'utf8');
  } catch (e) {
    return '';
  }
}
